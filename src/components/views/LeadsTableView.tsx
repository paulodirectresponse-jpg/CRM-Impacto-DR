...
export const LeadsTableView: React.FC = () => {
  ...
  // Sync with global leadFilterParams when navigated from another view
  useEffect(() => {
    if (leadFilterParams) {
      if (leadFilterParams.importBatchId) {
        setBatchFilter(leadFilterParams.importBatchId);
      }
      if (leadFilterParams.audienceId) {
        setAudienceFilter(leadFilterParams.audienceId);
      }
      if (leadFilterParams.search) {
        setSearch(leadFilterParams.search);
      }
      if (leadFilterParams.status) {
        setStatusFilter(leadFilterParams.status);
      }
    }
  }, [leadFilterParams]);
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    // Batch filter
    if (batchFilter && lead.importBatchId !== batchFilter) return false;
  ...
  const handleDownloadCsv = async () => {
    try {
      ...
      addToast({
        ...
        message: "O arquivo CSV de leads foi exportado com sucesso.",
      });
    } catch (err: any) {
      addToast({
        type: "error",
    ...
    <div id="leads-table-view" className="space-y-4 animate-in fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          <button
            id="btn-leads-table-prospect-now"
            onClick={() => setLeadViewMode("prospect")}
            className="text-xs flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-extrabold px-3.5 py-2 rounded-lg shadow-xs transition-all cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
            title="Iniciar sessão rápida de prospecção lead a lead"
          >
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="apify">Importado via Apify</option>
              <option value="paid">Tráfego Pago</option>
            </select>
          </div>
        </div>

        {/* View Archived Toggle & Active Batch Badge */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pt-2 border-t border-slate-100 text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {batchFilter && (
              <span className="inline-flex items-center gap-1.5 bg-indigo-50 text-indigo-700 border border-indigo-200 px-2.5 py-1 rounded-md font-semibold text-xs animate-in fade-in">
                <Layers className="w-3.5 h-3.5" />
                <span>Filtrando por Lote de Importação</span>
                <button
                  type="button"
                  onClick={handleClearBatchFilter}
                  className="hover:bg-indigo-100 rounded p-0.5 ml-1 transition-colors cursor-pointer"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-center">
                        ...
                        >
                          {lead.manualClass === "PENDENTE" ? "Pendente" : lead.manualClass === "RECUSADO" ? "Recusado" : `Classe ${lead.manualClass}`}
                        </span>
                      </td>
